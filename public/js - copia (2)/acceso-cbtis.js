  ...
  overlay.innerHTML = `
    ...
    <div class="flex items-center justify-between p-4 bg-black/30">
      ...
      <div class="flex items-center gap-4">
        ...
        <button onclick="cerrarKioscoCBTis()" class="p-3 bg-red-500/20 hover:bg-red-500/40 rounded-xl transition-all">
    ...
    <div class="flex-1 flex items-center justify-center p-4 gap-6">
      ...
      <div class="flex-1 max-w-2xl">
        <div class="relative bg-black rounded-3xl overflow-hidden shadow-2xl" style="aspect-ratio: 4/3;">
          ...
          <!-- Overlay de estado -->
          <div id="kiosco-estado" class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6">
            <p class="text-white text-xl text-center" id="kiosco-estado-texto">📷 Iniciando cámara...</p>
          </div>
          
          <!-- Indicador de modo -->
          <div id="kiosco-modo-indicador" class="absolute top-4 left-4 px-4 py-2 rounded-xl text-white font-bold text-lg">
            Selecciona modo
          </div>
        </div>
        ...
        <div class="flex gap-4 mt-6">
          <button onclick="setModoKiosco('Ingreso')" id="btn-kiosco-ingreso" 
                  class="flex-1 py-4 rounded-2xl text-white font-bold text-xl transition-all bg-emerald-600 hover:bg-emerald-500 border-4 border-transparent">
            🔓 ENTRADA
          </button>
          <button onclick="setModoKiosco('Salida')" id="btn-kiosco-salida"
                  class="flex-1 py-4 rounded-2xl text-white font-bold text-xl transition-all bg-red-600 hover:bg-red-500 border-4 border-transparent">
            🚪 SALIDA
          </button>
        </div>
      </div>
      ...
      <div class="w-80 h-full max-h-[600px] bg-white/5 rounded-3xl p-4 flex flex-col">
        <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
          📋 Últimos Registros
          <span id="kiosco-contador" class="ml-auto text-sm bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full">0</span>
        </h3>
        <div id="kiosco-lista-registros" class="flex-1 overflow-y-auto space-y-2">
          <p class="text-slate-500 text-center py-8">Sin registros aún</p>
        </div>
      </div>
    </div>
    
    <!-- Último reconocido -->
    <div id="kiosco-ultimo-reconocido" class="hidden fixed bottom-0 left-0 right-0 bg-gradient-to-r from-emerald-600 to-cyan-600 p-6 transform translate-y-full transition-transform duration-500">
      <div class="max-w-4xl mx-auto flex items-center gap-6">
        ...
        <div class="text-right">
          <p class="text-5xl font-bold text-white">✅</p>
          <p class="text-white/80" id="kiosco-hora-registro">--:--</p>
        </div>
      </div>
    </div>
  `;
...
/**
 * Muestra animación de éxito en kiosco
 */
function mostrarExitoKiosco(usuario, modo) {
  const panel = document.getElementById('kiosco-ultimo-reconocido');
  if (!panel) return;
  
  // Actualizar contenido
  document.getElementById('kiosco-avatar').textContent = usuario.nombre.charAt(0);
  document.getElementById('kiosco-nombre').textContent = `${usuario.nombre} ${usuario.apellidos || ''}`;
  document.getElementById('kiosco-info').textContent = `${usuario.tipo} ${usuario.grado ? '- ' + usuario.grado : ''} • ${modo}`;
  document.getElementById('kiosco-hora-registro').textContent = new Date().toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' });
  
  // Color según modo
  panel.className = modo === 'Ingreso'
    ? 'fixed bottom-0 left-0 right-0 bg-gradient-to-r from-emerald-600 to-cyan-600 p-6 transition-transform duration-500'
    : 'fixed bottom-0 left-0 right-0 bg-gradient-to-r from-red-600 to-orange-600 p-6 transition-transform duration-500';
...
function agregarRegistroListaKiosco(usuario, modo) {
  ...
  item.className = `p-3 rounded-xl ${modo === 'Ingreso' ? 'bg-emerald-500/20 border border-emerald-500/30' : 'bg-red-500/20 border border-red-500/30'}`;
  item.innerHTML = `
    <div class="flex items-center gap-3">
      <div class="w-10 h-10 rounded-full ${modo === 'Ingreso' ? 'bg-emerald-500' : 'bg-red-500'} flex items-center justify-center text-white font-bold">
        ${usuario.nombre.charAt(0)}
      </div>
      <div class="flex-1 min-w-0">
        <p class="text-white font-medium truncate">${usuario.nombre}</p>
        <p class="text-xs ${modo === 'Ingreso' ? 'text-emerald-400' : 'text-red-400'}">${modo} - ${new Date().toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}</p>
      </div>
    </div>
  `;
